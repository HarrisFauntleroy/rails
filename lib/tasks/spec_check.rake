...
namespace :spec_check do
  ...
  task models: :environment do
    Dir['app/models/**/*.rb'].each do |model_path|
      ...
      unless File.exist?(spec_path)
        puts Rainbow("Missing spec for model: #{model_path}").red
      end
    end
  end
  ...
  task controllers: :environment do
    Dir['app/controllers/**/*.rb'].each do |controller_path|
      ...
      unless File.exist?(spec_path)
        puts Rainbow("Missing spec for controller: #{controller_path}").blue
      end
    end
  end
  ...
  task views: :environment do
    Dir['app/views/**/*.erb'].each do |view_path|
      ...
      unless File.exist?(spec_path)
        puts Rainbow("Missing spec for view: #{view_path}").magenta
      end
    end
  end
  ...
  task all: :environment do
    ...
    check_types.each do |type, config|
      Dir[config[:path]].each do |file_path|
        ...
        unless File.exist?(spec_path)
          missing_specs << "#{type} spec missing for: #{file_path}"
        end
      end
    end
